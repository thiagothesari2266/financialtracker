...
import { CheckCircle, AlertTriangle, Edit3, Trash2, Calendar, DollarSign, Tag } from 'lucide-react';
...
export default function TransactionReviewModal({
  ...
  const findMatchingCategory = (categoryName: string, description: string = '') => {
    ...
    let match = categories.find((cat) => combinedText.includes(cat.name.toLowerCase()));
    ...
    for (const mapping of smartMappings) {
      if (
        ...
        match = categories.find((cat) => cat.name.toLowerCase() === mapping.category.toLowerCase());
        if (match) return match;
      }
    }
  ...
  const handleSelectTransaction = (id: string, checked: boolean) => {
    setTransactions((prev) => prev.map((tx) => (tx.id === id ? { ...tx, selected: checked } : tx)));
  ...
  const handleEditTransaction = (id: string, field: string, value: any) => {
    setTransactions((prev) =>
      prev.map((tx) => (tx.id === id ? { ...tx, [field]: value, edited: true } : tx))
    );
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              <strong>Resumo:</strong> {transactions.length} transações encontradas. Revise as
              informações e selecione quais deseja importar.
            </AlertDescription>
          </Alert>
                    ...
                    {transaction.launchType === 'recorrente' && (
                      <div className="grid grid-cols-2 gap-4 text-sm text-purple-700 bg-purple-50 p-2 rounded">
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {formatRecurrenceFrequency(transaction.recurrenceFrequency || '')}
                          </span>
                        </div>
                    ...
                    {editingTransaction === transaction.id && (
                      <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded border">
                        ...
                        <div className="col-span-2">
                          <Label htmlFor={`launch-type-${transaction.id}`}>
                            Tipo de Lançamento
                          </Label>
                          <Select
                        ...
                        {transaction.launchType === 'recorrente' && (
                          <>
                            <div>
                              <Label htmlFor={`recurrence-frequency-${transaction.id}`}>
                                Frequência
                              </Label>
                              <Select
                            ...
                            <div>
                              <Label htmlFor={`recurrence-end-${transaction.id}`}>
                                Data Final (opcional)
                              </Label>
                              <DatePicker
                                date={
